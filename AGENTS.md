...
- 내부 장면 이동의 기준 경계는 광고 상단 80% 시작과 하단 20% 종료다. `src/inner-scene-scroll.js`
- 고정 장면 전환은 300vh 섹션의 sticky travel을 0~1 progress로 계산한다. `src/fixed-scene-transition.js`
...
- [x] 고정 장면 전환 패턴 — `src/fixed-scene-transition.js`와 `#/patterns/fixed-scene-transition`
