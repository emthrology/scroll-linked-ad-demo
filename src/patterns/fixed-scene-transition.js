...
export function createFixedSceneTransition({ container, sceneCount, onUpdate }) {
  ...
  let frameId = 0
  const update = () => { frameId = 0; const rect = container.getBoundingClientRect(); const travel = Math.max(1, rect.height - window.innerHeight); const progress = clamp(-rect.top / travel, 0, 1); const stage = Math.min(sceneCount - 1, Math.floor(progress * sceneCount)); container.style.setProperty('--fixed-progress', progress); onUpdate?.({ progress, stage }) }
